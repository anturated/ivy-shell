...
// shamelessly stolen from caelestia
...
Singleton {
    ...
    readonly property string icon: cc ? getWeatherIcon(cc.weatherCode) : "cloud_alert"
    ...
    readonly property string sunrise: cc ? Qt.formatDateTime(new Date(cc.sunrise), Settings.main.useTwelveHourClock ? "h:mm A" : "h:mm") : "--:--"
    readonly property string sunset: cc ? Qt.formatDateTime(new Date(cc.sunset), Settings.main.useTwelveHourClock ? "h:mm A" : "h:mm") : "--:--"

    readonly property var cachedCities: new Map()

    function reload(): void {
        const configLocation = Settings.main.weatherLocation;
        ...
        Requests.get(url, text => {
            ...
            for (let i = 0; i < json.hourly.time.length; i++) {
                ...
                hourlyList.push({
                    ...
                    icon: getWeatherIcon(json.hourly.weather_code[i])
                });
            }
